(function () {
  ...
  function inject() {
    ...
    document.body.insertAdjacentHTML('beforeend',
      '<button class="tf-chat-launch" type="button" aria-label="Messages">✉<span class="tf-chat-badge" data-badge></span></button>' +
      '<aside class="tf-chat-widget" data-open="false" data-pane="list" aria-label="Messages">' +
      '<section class="tf-chat-list"><div class="tf-chat-head"><strong>Messages</strong><button class="tf-chat-close" data-close aria-label="Close">×</button></div><div data-conversations></div></section>' +
      '<section class="tf-chat-thread"><div class="tf-chat-head"><span><button class="tf-chat-close tf-chat-back" data-back style="display:none" aria-label="Back">‹</button> <strong data-title>Conversation</strong></span><span class="tf-chat-tools"><button class="tf-chat-close" data-min aria-label="Minimize">−</button><button class="tf-chat-close" data-max aria-label="Maximize">□</button><button class="tf-chat-close" data-close aria-label="Close">×</button></span></div><div class="tf-chat-messages" data-messages><p class="tf-chat-empty">Choose a conversation</p></div><form class="tf-chat-compose"><input maxlength="4000" aria-label="Message" placeholder="Write a message…"><button class="tf-button" type="submit">Send</button></form></section></aside>');
  }
  function renderList() {
